...
bool Tokenizer::is_negative(char c) {
  return c == '-' ;
 }
...
Token Tokenizer::number() {
  ...
  if ( buffer[i] == '.') {
    s += buffer[i++];
    while ( is_num(buffer[i]) || buffer[i] == '.'|| buffer[i] == '-') {
      s += buffer[i++];
    }
  }
  ...
  if ( buffer[i] == '-' ) {
    s += buffer[i++];
    while ( is_num(buffer[i]) || buffer[i] == '.') {
      s += buffer[i++];
    }
  }
...
Token Tokenizer::next() {
  ...
  case 'f':
    ...
    return Token(false);
  default:
    if ( is_num(buffer[i]) || buffer[i] == '.'|| buffer[i] == '+' || buffer[i] == '-'|| buffer[i] == 'e') {
      return number();
    } else {
